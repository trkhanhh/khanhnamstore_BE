...
public class ProductCommentApi {
    ...
    @GetMapping("/public/most-comment")
    public ResponseEntity<?> getMostOfComment(){
        List<ProductCommentResponse> response = productCommentService.getMostComment();
        return new ResponseEntity<>(response.subList(0,response.size() > 10  ? 10 : response.size() ),HttpStatus.OK);
    }
